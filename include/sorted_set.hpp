...
/// 集合・多重集合の共通実装。Compare は狭義弱順序を満たす必要があります。
...
public:
    ...
    /// values を compare 順に構築します。Multi が false なら重複を除去します。
    ...
    /// 初期化リストを compare 順に構築します。集合では重複を除去します。
    ...
    /// 半開区間 [first, last) を compare 順に構築します。集合では重複を除去します。
